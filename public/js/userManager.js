...
const confirmDelete = async () => {
    ...
    try {
        const response = await fetch(`http://3.36.118.177:3000/users/withdraw/${userInfo.userId}`, {
            method: "DELETE",
        ...
        try {
            ...
            if (nickname && nickname !== userInfo.nickname) {
                if (nickname.length > 10) {
                    ...
                    // 닉네임 업데이트 요청
                    const nicknameResponse = await fetch(`http://3.36.118.177:3000/users/nickname/${userInfo.userId}`, {
                        method: "PATCH",
            ...
            if (file) {
                ...
                if (!uploadResponse.ok) {
                    ...
                    // 프로필 이미지 URL 업데이트 요청
                    const imageResponse = await fetch(`http://3.36.118.177:3000/users/profileImg/${userInfo.userId}`, {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
