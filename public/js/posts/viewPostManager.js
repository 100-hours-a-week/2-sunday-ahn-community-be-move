document.addEventListener("DOMContentLoaded", async () => {
    ...
    const fetchPostDetails = async (postId) => {
        try {
            const response = await fetch(`http://3.36.118.177:80/posts/${postId}`, {
                method: 'GET',
...
const confirmDelete = () => {
    ...
    // 백엔드로 삭제 요청 보내기
    fetch(`http://3.36.118.177:80/posts/${postId}`, {
        method: "DELETE",
        credentials: 'include' // 세션 쿠키를 포함시킴
    })
    .then(response => {
...
const toggleLike = async () => {
    ...
    try {
        const response = await fetch(`http://3.36.118.177:80/posts/${postId}/likes`, {
            method: 'GET',
            credentials: 'include'
        });
